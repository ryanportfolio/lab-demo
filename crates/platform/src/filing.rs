...
fn normalize(rel: &mut [f64], zone_exposure: &[f64]) {
    ...
    let mean: f64 =
        (0..rel.len()).map(|z| zone_exposure[z] * rel[z]).sum::<f64>() / total;
...
/// (zone 0 = 1.0 before normalization) and the zone exposures of those rows.
pub fn raw_relativities(rows: &[&PolicyRow]) -> Result<(Vec<f64>, Vec<f64>), String> {
    let spec = ModelSpec {
...
pub fn filed_relativities(
    filing_rows: &[&PolicyRow],
    k: f64,
) -> Result<Vec<f64>, String> {
    let (mut raw, zone_exposure) = raw_relativities(filing_rows)?;
